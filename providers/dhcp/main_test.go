...
func (s *DHCPS) SetupSuite() {
	...
	v.Set("leaseDuration", 24*time.Hour)
	v.Set("dnsServers", []string{"10.0.0.1", "10.0.0.2"})
...
func (s *DHCPS) TestConfig() {
	t := []struct {
		name     string
		getter   func(*Config) (interface{}, error)
		expected interface{}
	}{
		{"lease duration",
			func(c *Config) (interface{}, error) { return c.LeaseDuration() },
			24 * time.Hour,
		},
		{"gateway",
			func(c *Config) (interface{}, error) { return c.Gateway() },
			net.ParseIP("10.0.0.1"),
		},
		{"network",
			func(c *Config) (interface{}, error) { return c.Network() },
			func() *net.IPNet { _, network, _ := net.ParseCIDR("10.0.0.1/24"); return network }(),
		},
		{"dns servers",
			func(c *Config) (interface{}, error) { return c.DNSServers() },
			[]net.IP{net.ParseIP("10.0.0.1"), net.ParseIP("10.0.0.2")},
		},
	}

	for _, t := range t {
		got, err := t.getter(s.config)
		s.NoError(err)
...
func (s *DHCPS) TestGetAddressBasic() {
	...
	lease := resp.(Lease)
	s.Require().Equal(s.dhcp.duration, lease.Duration)
	s.Require().Equal(s.dhcp.gateway, lease.Gateway)
	s.Require().Equal(s.dhcp.network.Mask, lease.Net.Mask)
	s.Require().Equal(s.dhcp.network.IP, lease.Net.IP.Mask(lease.Net.Mask))
...
func (s *DHCPS) TestGetTaken() {
	...
	lease2 := resp.(Lease)
	s.Require().Equal(s.dhcp.duration, lease2.Duration)
	s.Require().Equal(s.dhcp.gateway, lease2.Gateway)
	s.Require().Equal(s.dhcp.network.Mask, lease2.Net.Mask)
	s.Require().Equal(s.dhcp.network.IP, lease2.Net.IP.Mask(lease2.Net.Mask))
