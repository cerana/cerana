...
func New(config *Config, tracker *acomm.Tracker) (*DHCP, error) {
	gateway, err := config.Gateway()
	if err != nil {
		return nil, err
	}

	network, err := config.Network()
	if err != nil {
		return nil, err
	}

	duration, err := config.LeaseDuration()
	if err != nil {
		return nil, err
	}

	dnses, err := config.DNSServers()
	if err != nil {
		return nil, err
	}
...
func (d *DHCP) get(req *acomm.Request) (interface{}, *url.URL, error) {
	...
	if addrs.IP != "" {
		...
		if mac == addrs.MAC {
			...
			if ok {
				...
				fmt.Println("renewed lease:", lease)
	...
	if len(ips) == 0 || ips[0] > ipToU32(d.network.IP)+1 {
		ip := dhcp4.IPAdd(d.network.IP, 1)
	...
	defer close(closer)
	for uIP := range nextGetter(closer, ips, ipToU32(d.network.IP), ipToU32(d.maxIP)) {
		ip := u32ToIP(uIP)
...
func (d *DHCP) ack(req *acomm.Request) (interface{}, *url.URL, error) {
	...
	return nil, nil, refreshLeaseAck(d.tracker, d.coordinator, addrs.MAC, addrs.IP, d.duration)
}
