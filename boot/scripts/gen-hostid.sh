...
# Parts of this script are based on one from Fazle Arefin
...
echo -ne "\\x$a\\x$b\\x$c\\x$d" >/etc/hostid \
    && echo "Set hostid to $host_id"

echo "$machine_id" >/etc/machine-id \
    && echo "Set machine-id to $machine_id"

echo "$uuid" >/etc/machine_id \
    && echo "Set machine_id to $uuid"

echo "$uuid" >/etc/hostname \
    && echo "Set hostname to $uuid"
